...
5. [Transactional Outbox Pattern](#transactional-outbox-pattern)
...
```
GIN(GENERALIZED INVERTED INDEX) 인덱스는 역색인 구조로 '포함(Contains)연산'에 최적화되어 기존 B-Tree 인덱스로 해결하기 어려운 특정 유형의 쿼리 성능을 극적으로 향상시킨다.
```

**역색인(Inverted Index)이란?**

기존 색인(Index)의 경우 "이 문서엔 뭐가 있지?"라는 개념으로 접근한다.</br>
문서 ID(Row id) -> 포함된 단어 목록의 방식이다.</br>
예를들어 전화번호부의 이름을 검색한다고 가정했을 때 '문서 1페이지 -> BOB, 문서 2페이지 -> Alice' 같은 방식이다.</br>
특정 문서(또는 Row)에 어떤 내용이 있는지 확인할 때 유용하지만, 특정 데이터가 포함된 모든 문서를 찾을경우 색인을 다 뒤져봐야하기 때문에 비효율적이다. 

역색인의 경우 '단어'를 기준으로 그 단어가 어떤 '문서'들에 나타나는지의 개념으로 접근한다.</br>
예를들어 '동물사전에서 강아지가 포함된 문서는 1페이지, 5페이지 / 고양이가 포함된 문서는 3페이지, 8페이지' 같은 방식이다.</br>
특정 단어가 포함된 문서를 찾을 때 압도적으로 빠르며, 그렇기 때문에 검색 엔진처럼 키워드 기반의 검색에서 효율적이다.

**왜 GIN이 필요한가?**

흔히 SQL에서 `LIKE %keyword%` 같은 쿼리를 사용하면서 인덱스를 타지 못하고 Full Scan을 하는 경우를 많이 경험했을것이다. 이런 문제를 해결하기 위해서 나온것이 GIN이다.

예를들어 '주소'라는 컬럼이 있다고 가정해보자.</br>
만약 일반적인 B-Tree 인덱스를 사용했을경우 `LIKE 서울특별시%`로 검색을 하면 어떻게 될까? 이 경우에는 인덱스를 효율적으로 사용하므로 검색이 빠르다.</br>
하지만 `LIKE %강남대로%`식의 검색어가 와일드카드(%)로 시작한다면, 정렬된 B-Tree 인덱스를 전혀 활용하지 못하고, FullSacn을 수행하게 된다.

이 경우에 GIN을 사용하면 아주 효율적인 검색이 가능하다.</br>
먼저 '주소'컬럼을 전문 검색용 데이터 타입인 `tsvector`로 변환하고, 여기에 GIN 인덱스를 생성한다.</br>
이 인덱스는 주소를 '서울시', '강남구', '강남대로' 같은 의미 있는 단어(Lexeme) 단위로 쪼개서 인덱싱 한다.</br>
'강남대로'를 키워드 검색할 경우 다음과 같이 동작한다.

1. 사용자의 검색어 '강남대로'를 `tsquery`로 변환
2. GIN 인덱스에서 '강남대로'라는 키를 찾기
3. 해당 키가 가리키는 주소(row)의 위치 값을 응답

**모든곳에 필요할까?**

GIN 인덱스가 만능인것 같지만, 장점과 더불어 단점도 많다.</br>
단점을 살펴보자면

1. 쓰기(INSERT,UPDATE)작업에 속도 저하가 발생한다.</br>
	*  B-Tree 인덱스는 데이터가 하나 추가되면 인덱스에도 하나의 항목이 추가되지만, GIN 인덱스는 다르다.
	* 예를들어,tag 컬럼에 {'DB','PostgreSQL','Index'}라는 배열이 새로 추가되면, GIN 인덱스는 3개의 키에 대해 모두 인덱스를 업데이트해야 한다. 하나의 INSERT가 d여러번의 인덱스 쓰기 작업을 유발하는 셈이다.
2. 인덱스의 크기가 크다
	* B-Tree는 '전체 값'하나만 저장하면 되지만, GIN은 값을 구성하는 '모든 요소'를 각각 저장하고, 각 요소마다 어떤 row에 포함되어 있는지에 대한 위치 정보 목록(Posting List)를 유지해야 하므로 상대적으로 인덱스가 차지하는 디스크 공간이 커지게 된다. 
3. 특정 종류의 쿼리에는 부적합하다.
	* 값의 동등성(=)이나 범위 검색에는 B-Tree보다 오히려 느릴 수 있다.

</br>

```
GIN은 배열 검색, JSONB 검색, 전문 검색등에 특화되어 있고,
포함 여부/존재 여부 연산을 빠르게 처리하여 B-Tree 인덱스로 해결하기 어려운 쿼리를 빠르게 수행한다.

다만 쓰기(INSERT, UPDATE)작업에서의 속도 저하, 커다란 인덱스의 크기, 특정 종류의 쿼리에 부적합 하므로 상황에 맞게 사용하는것이 중요하다.
```

### Transactional Outbox Pattern
```
TOP(Transactional Outbox Pattern)는 MSA와 같은 분산 시스템에서 서비스 간 데이터 일관성을 보장하기 위해 사용되는 설계 패턴.

이 패턴은 서비스의 내부 데이터베이스 변경과 외부로의 메시지 발행을 하나의 트랜잭션으로 묶어 원자성을 확보함으로써, 두 작업 중 하나만 성공하고 하나는 실패하는 상황을 방지한다.
```

**왜 TOP가 필요한가?**

일반적으로 MSA에서는 각 서비스가 자신만의 데이터베이스를 소유하고, 한 서비스의 상태 변경을 다른 서비스에 알려야 하는 경우가 많다.</br>
흔히 사용되는 주문-결제-배송 형태의 MSA에서 주문 서비스가 데이터베이스에 주문 정보를 저장한 직후, 메시지 브로커(RabbitMQ, SQS, Kafka)에 '주문 생성됨'이벤트를 발행하는 것이다. 하지만 여기서 다음과 같은 문제가 발생할 수 있다.

* 데이터베이스 저장에는 성공했지만, 메시지 발행에 실패한 경우: 주문은 생성되었지만 다른 서비스들은 이 사실을 알지 못해 다음 프로세스(결제, 배송 등)가 진행되지 않는다.
* 메시지 발행에는 성공했지만, 데이터베이스 저장이 실패(롤백)된 경우: 실제 주문 데이터는 없는데도 다른 서비스들은 주문이 생성되었다고 착각하여 잘못된 처리를 할 수 있따.

이처럼 두개의 독립적인 시스템(데이터베이스, 메시지 브로커)에 걸쳐 작업을 원자적으로 처리하기란 쉽지 않다. 이를 이중 쓰기(Dual Write)문제라고 하며, TOP는 이 문제를 해결하기 위해 고안되었다.


**TOP의 동작원리**

TOP의 핵심은 아웃박스(Outbox)라는 별도의 테이블을 서비스 데이터베이스 내에 두는것이 핵심이다.</br>
작동 방식은 다음과 같다.

1. 단일 트랜잭션으로 처리: 애플리케이션은 비즈니스 로직 처리(eg:주문 정보 저장)와 아웃박스 테이블에 메시지(이벤트)를 저장하는 작업을 하나의 트랜잭션으로 묶어서 실행한다.
	* 만약 비즈니스 로직 처리가 실패하면, 아웃박스 테이블에 메시지를 저장하는 작업도 함께 롤백된다.
	* 반대로 아웃박스 테이블 저장에 실패해도 전체 트랜잭션이 롤백되어 데이터 불일치가 발생하지 않는다.
2. 메시지 릴레이(Message Relay): 별도의 비동기 프로세스(메시지 릴레이)가 아웃박스 테이블을 주기적으로 폴링(polling)하거나
데이터베이스의 변경 데이터 캡쳐(CDC, Change Data Capture)기능을 사용하여 새로운 메시지가 있는지 확인한다.
3. 메시지 발행 및 상태 업데이트: 메시지 릴레이는 아웃박스 테이블에서 아직 발행되지 않은 메시지를 가져와 메시지 브로커로 발행한다.
	* 메시지 발행에 성공하면, 해당 메시지가 처리되었음을 표시하기 위해 아웃박스 테이블의 상태를 업데이트한다. 이는 동일한 메시지가 중복으로 발행되는것을 방지한다.

**Message Relay 구현 기술**

프로덕션 레벨에서 메시지 릴레이를 구현하는 방식은 크게 폴링(Polling) 방식과 CDC(Change Data Capture) 방식으로 나뉜다.

1. 폴링(Polling) 방식: 단순하고 직관적인 구현</br>
이 방식은 별도의 스케줄러나 백그라운드 작업이 주기적으로 아웃박스 테이블을 조회하여 발행되지 않은 메시지를 찾아 메시지 브로커로 전송하는 구조
	* 장점: 구현이 비교적 간단하고, 외부 시스템에 대한 의존성이 적다
	* 단점: 불필요한 DB 조회가 계속 발생하여 부하를 줄 수 있고, 폴링 주기에 따라 실시간성이 저하될 수 있다.
	* 주요 구현 기술:
		* @Schedule 어노테이션: 스프링 스케줄러를 이용해 특정 시간 간격으로 아웃박스 테이블을 조회하는 메서드를 실행
		* Spring Batch: 정해진 시간마다 Job을 실행하여 아웃박스 테이블의 메시지를 읽어와(Reader) 처리하고(Processor) 메시지 브로커에 발행(Writer)하는 방식으로 구현
		* Quartz Scheduler: 정교한 스케줄링이 필요할 때 사용
		* 애플리케이션 내 백그라운드 스레드: 별도의 스레드를 만들어 while루프를 이용해 직접 폴링 로직을 구현
2. 변겅 데이터 캡처(CDC)방식: 실시간에 가까운 고성능 구현</br>
CDC는 데이터베이스의 트랜잭션 로그(Transaction LOG)를 감시하여 데이터 변경(INSERT, UPDATE, DELETE)이 발생했을 때 이를 이벤트로감지하고 스트리밍 하는 기술이다.
아웃박스 테이블에 새로운 메시지가 INSERT되는 것을 실시간으로 감지할 수 있어 폴링 방식의 단점을 보완한다.
	* 장점: 데이터베이스에 직접적인 부하를 주지 않으면서 변경을 실시간에 가깝게 감지할 수 있어 성능이 뛰어나다
	* 단점: 폴링 방식에 비해 초기 구성이 복잡하고, Debezium, Kafka Connect 등 추가적인 기술 스택에 대한 이해가 필요하다.
	* 주요 구현 기술: Debezium + Kafkaconnect가 현재 표준적으로 사용되는 조합
	* 동작 흐름:
		* 서비스가 비즈니스 로직과 함께 아웃박스 테이블에 메시지를 INSERT(단일 트랜잭션)
		* Debezium 커넥터가 데이터베이스 트랜잭션 로그를 통해 이 INSERT 이벤트를 감지
		* Debezium이 변경 이벤트를 생성하여 Kafka 토픽으로 전송
		* (선택적) Kafka Streams나 별도의 메시지 릴레이 애플리케이션이 이 토픽을 구독하여 메시지를 최종 목적지로 라우팅하거나 포맷을 변경

**꼭 MSA에서만 사용되나?**
TOP의 핵심은 '데이터베이스 트랜잭션과 그 결과로 수행되어야 할 후속 조치(Side Effect)를 분리하는 것'이다.</br>
이 후속 조치가 MSA에서는 '다른 서비스에 메시지 보내기'일 뿐, 모놀리식에서는 다른것이 될 수 있다.

가장 대표적인 사례는 '외부 시스템 API 호출 및 비동기 작업 처리'이다.</br>
예를 들어, 다음과 같은 회원 가입 시나리오를 생각해보자

1. 데이터베이스에 사용자 정보를 저장
2. 가입 환영 이메일을 발송
3. 가입 축하 쿠폰을 지급하기 위해 프로모션 시스템 API를 호출

만약 이 세가지를 하나의 로직에서 순차적으로 처리하면 다음과 같은 문제가 발생할 수 있다.
* 결합성 문제: 이메일 발송 서버가 느리거나(Latency), 쿠폰 시스템이 일시적으로 장애가 발생하면(Unavailability) 전체 회원가입 트랜잭션이 지연되거나 실패한다. 사용자는 단지 회원가입을 했을 뿐인데, 부가 기능의 장애 때문에 핵심 기능에 영향을 받는다.
* 일관성 문제: DB에 사용자 정보 저장은 성공했는데, 그 직후에 애플리케이션이 다운되어 이메일 발송이나 쿠폰 지급에 실패하면 데이터가 불일치하게 된다.

이런 상황에서 TOP를 적용하면 문제를 해결할 수 있다.
1. 하나의 트랜잭션 처리: 사용자 테이블에 사용자 정보를 저장하고, outbox 테이블에 '이메일 발송', '쿠폰 지급'이라는 두 개의 이벤트를 저장하는 것을 하나의 DB 트랜잭션으로 묶는다. 이로써 사용자의 요청(회원가입)은 매우 빠르게 처리되고 DB에 안전하게 저장된다.
2. 비동기 후속 처리: 별도의 백그라운드 워커(메시지 릴레이)가 아웃박스 테이블을 폴링하거나 CDC로 감지하여 이벤트를 처리
	* '이메일 발송' 이벤트를 보고 메일 서버 API를 호출
	* '쿠폰 지급' 이벤트를 보고 프로모션 시스템 API를 호출

이렇게 하면 이메일 서버나 쿠폰 시스템이 잠시 불안정하더라도 회원가입이라는 핵심 기능은 전혀 영향을 받지 않는다. 백그라운드 워커가 재시도 로직을 통해 안정적으로 후속 조치를 완료할 수 있다.

```
결론적으로 Transactional Outbox Pattern의 가치는 신뢰성이 중요한 핵심 DB 트랜잭션과 신뢰성이 낮거나 시간이 오래걸리는 부가적인 작업을 안정적으로 분리하는데 있다.

분산 시스템에서는 그 부가 작업이 '다른 서비스로의 메시지 발행'인 것이고,
모놀리식 시스템에서는 '외부 API 호출','이메일/SMS 발송','느린 파일 처리'등이 될 수 있다.
```
